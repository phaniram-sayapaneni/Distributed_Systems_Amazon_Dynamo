...
public class SimpleDynamoProvider extends ContentProvider {
	...
	//static List<String> succs = new ArrayList<String>();
		...
		try {
			...
			if((haskey.compareTo(myhash)<=0 && haskey.compareTo(predhash)>0)  ||  park_node.equals(portnum))
			{
				...
				try {
					...
					Log.d("filewrite" , "what i'm writing: key:" + key + "value:" + string );
					fw.write(string);
					fw.close();

                   ///  put key values to your replicass first
					//String for_park_node =  "blindinsert" + " " + "key" + " "+ key + " "+ string;
					//String [] 	replicas = return_succs(park_node);
			...
			else
			{
				...
				//new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, for_park_node, replicas[0]);

				//new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, for_park_node, replicas[1]);
	...
	public boolean onCreate() {
		...
		try {
			...
			Log.d("recovery", "wtf is wrong with you??" + "portnum: " + portnum);
		} catch (IOException e) {
			...
			//nodes.add("11108"); nodes.add("11112"); nodes.add("11116"); nodes.add("11120"); nodes.add("11124");
			//Collections.sort(nodes, new seqcomparator());
			//int mynum =Integer.parseInt(portnum);

		/// now after it recovers take all files from its friends and store the ones which it is supposed to store



		///
		//String [] mypreds2 = two_level_find_prev_node(portnum);
		Log.d("recovery", "wtf is wrong with you??" + "now what again" );
			...
			for (int i = 0; i < 5; i++) {
				if ((!all_f[i].isEmpty()) && (!all_f[i].equals("")) && all_f[i].length() > 2) {
					...
					for (int j = 0; j < allmsgs.length; j += 2) {
						...
						//mc.addRow(rw);
						//j = j+1;
						//String [] preds = two_level_find_prev_node(portnum);
						//String park_here = find_key_parking(rw[0]);
						//if(park_here.equals(preds[0]) || park_here.equals(preds[1]) ||park_here.equals(portnum)) {
						if(rw[0].substring(0,4).equals("null")) rw[0] = rw[0].substring(4);
							String filename = rw[0] + ".txt";
							String string = rw[1];
							File file = new File(context.getFilesDir(), filename);
							FileWriter fw = new FileWriter(file);
							Log.d("filewrite", "what i'm writing: key:" + filename + "value:" + string);
							fw.write(string);
							fw.close();
						//}
		...
		///////
          ...
          // from succs not required
		/*
		String [] mysuccs2 = return_succs(portnum);
		String [] all_f2 = new String[2];

		Log.d("recovery", "trying recovery");

		try {
			all_f2[0] = new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, "starquery", mysuccs2[0]).get();
			Log.d("recovery", "here is all_f[0]:" + all_f[0]);
			all_f2[1] = new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, "starquery", mysuccs2[1]).get();
			Log.d("recovery", "here is all_f[0]:" + all_f[1]);



			for (int i = 0; i < 1; i++) {
				if ((!all_f[i].isEmpty()) && (!all_f[i].equals("")) && all_f[i].length() > 2) {
					String[] allmsgs = all_f2[i].split(" ");
					//Log.d("starquery", "here is the msg from s[i]" + s[i]);
					for (int j = 0; j < allmsgs.length; j += 2) {
		...
		*/
		...
		//// now take it from succs also
		/// now you need to delete files which are absent in both
		/*

		try {
			Context context = getContext();
			String [] pres2 = two_level_find_prev_node(portnum);
			...
			for (int i = 0; i < listoffiles.length; i++)
			{
				...
				String eachfile2 = eachfile.replace(".txt", "");
				String parkk = find_key_parking(eachfile2);
				if(parkk.equals(pres2[1]))
				{
				//String [] pres2 = two_level_find_prev_node(parkk);
				String askifpresent = "present?" + eachfile;
				String ans1 = new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, askifpresent, parkk).get();
				String ans2 = 	new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, askifpresent, pres2[0]).get();

				if(ans1.equals("no") && ans2.equals("no"))
				{
					/// now delete this file
					//Context context2 = getContext();
					//Log.d("inside_delete", " deleting this file" + selectio);
					File fin2 = new File(context.getFilesDir(), (eachfile));
					fin.delete();
				}
				...
				else if(parkk.equals(pres2[0]))
				{
					String [] s2 = return_succs(portnum);
					String askifpresent = "present?" + eachfile;
					String ans1 = new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, askifpresent, parkk).get();
					String ans2 = new SimpleDynamoProvider.ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, askifpresent, s2[0]).get();
	...
	{   //Collections.sort(nodes, new seqcomparator());
		...
		//{
	//		Log.d("nodes", "key:" + key + "node:" + nodes.get(i));
	//	}
	...
	public String find_prev_node(String curr_node)
	{
		...
		/*
		int index = nodes.indexOf(curr_node);
		if(index == 0) return nodes.get(nodes.size()-1);
	...
	public void generate_succs()
	 {
		 ...
		 /*
		 Collections.sort(nodes, new seqcomparator());	 // just to make sure things are removed out
		 if(nodes.size() == 5)
		 {
			 for (int i = 0; i < 5; i++)
			 {
				 ...
				 if (nd.equals(portnum))
				 {
					 ...
					 if (i == 4) {
						 ...
						 succs[0] =   (nodes.get(nodes.size() - 1));
						 succs[1] = (nodes.get(0));
					 }

					  else {succs[0] = (nodes.get(i)); succs[1] = (nodes.get(i+1));
					          }
				 }
				 }
			 ...
			 for (int i = 0; i < 4; i++)
			 {
				 ...
				 if (nd.equals(portnum))
				 {
					 if (i == 3)
					 {
						 ...
						 succs[0] = (nodes.get(i)); succs[1] = (nodes.get(i+1));
					 }
				 }
		...
		try {
			...
			else
			{
				...
				try {
					...
					/*
					if ((!s[0].isEmpty()) && (!s[0].equals("")) && s[0].length() > 1)    //if ((!s[0].isEmpty()) && (!s[0].equals("")) && s[0].length() > 1)
					{
		...
		protected String doInBackground(ServerSocket... sockets) {
			...
			try {
				...
				while (1 == 1) {
					...
					if(msgr.length == 4)
					{
						...
						if(msgr[0].equals("blindinsert") || msgr[0].equals("replica_forwarding"))
						{
							try {
								...
								FileWriter fw =  new FileWriter(file);
								Log.d("filewrite" , "what i'm writing: key:" + msgr[2] + "value:" + msgr[3] );
								fw.write(msgr[3]);
								fw.close();
					...
					////////////tell if present or not ends here
					/// did i miss anything starts here
